...
import { ChromeDebugAdapter, logger, utils } from "vscode-chrome-debug-core";
...
export class IOSDebugAdapter extends ChromeDebugAdapter {
	...
	public launch(args: any): Promise<void> {
		if (args.port == null) {
			return utils.errP(
				'The "port" field is required in the launch config.',
			);
		}
		...
		return this._spawnProxy(<IProxySettings>settings)
			...
			.then((url) => {
				...
				(<any>this)._chromeConnection.sendMessage("Page.navigate", {
					url: url,
				});
			});
	}
	...
	public attach(args: any): Promise<void> {
		if (args.port == null) {
			return utils.errP(
				'The "port" field is required in the attach config.',
			);
		}
	...
	private _attachToDevice(
		proxyPort: number,
		deviceName: string,
	): Promise<number> {
		// Attach to a device over the proxy
		return utils.getURL(`http://localhost:${proxyPort}/json`).then(
			(jsonResponse) => {
					...
					if (Array.isArray(responseArray)) {
						...
						if (devices.length) {
							if (devices.length > 1 && deviceName !== "*") {
								logger.log(
									`Warning: Found more than one valid target device. Attaching to the first one. Available devices: ${JSON.stringify(devices.map((d) => d.deviceName))}`,
								);
							}
